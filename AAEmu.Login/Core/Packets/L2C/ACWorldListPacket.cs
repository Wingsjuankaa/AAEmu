    ...
    : LoginPacket(LCOffsets.ACWorldListPacket)
{
    ...
    {
        stream.Write(true); // AA8 expects the privacy policy state before the world count.
        ...
        foreach (var gs in gameServers)
        {
            ...
            stream.Write((ushort)1);          // AA8 world title: fresh/new
            stream.Write((byte)1);            // AA8 title color
            ...
            if (gs.Active)
            {
                ...
                for (var i = 0; i < 9; i++)   // AA8 exposes nine race slots here
                    stream.Write((byte)0);
                /*
                 RACE_NONE = 0,
                 RACE_NUIAN = 1,
