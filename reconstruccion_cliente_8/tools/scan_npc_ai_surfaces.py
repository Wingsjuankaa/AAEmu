...
def build_snapshot(
    *,
    ...
    roots: list[tuple[Path, set[str], str]] = [
        *((root, {".dll", ".exe"}, "client_binary") for root in binary_roots),
    ...
    for root, suffixes, kind in roots:
        ...
        scans.append(scan)
    return {
        "format": "AA8_NPC_AI_SURFACE_SNAPSHOT_V1",
        "tokens": list(TOKENS),
        "scans": scans,
        "totals": {
            "files_scanned": sum(
                int(scan["files_scanned"]) for scan in scans
            ),
            "bytes_scanned": sum(
                int(scan["bytes_scanned"]) for scan in scans
            ),
            "matching_files": sum(
                len(scan["matches"]) for scan in scans
            ),
        },
    }
