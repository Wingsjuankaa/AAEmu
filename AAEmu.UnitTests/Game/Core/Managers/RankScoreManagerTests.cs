...
public class RankScoreManagerTests
{
    ...
    [Test]
    public async Task PayEndedWindows_DeferredDeliveryDoesNotMarkWindowPaid()
    {
        var store = new InMemoryStore();
        var manager = new RankScoreManager(store, Mock.Of<ITaskManager>().Object);
        using var data = SeededBoard(permitTie: false);
        ...
        await Assert.That(store.HasPayout(board.Id, previous.StartUtc)).IsFalse();
    }

    [Test]
    public async Task PayEndedWindows_RepeatedPassPreservesUnpaidWindow()
    {
        var store = new InMemoryStore();
        var manager = new RankScoreManager(store, Mock.Of<ITaskManager>().Object);
        using var data = SeededBoard(permitTie: false);
        ...
        await Assert.That(store.PayoutMarks.Count(pair => pair.RankId == 42 && pair.Period == previous)).IsEqualTo(0);
    }
