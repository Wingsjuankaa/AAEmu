...
public class CombatResourceCeilingTests
{
    ...
    [Test]
    public async Task ADeltaThatClosesAKnownPool_DoesNotMakeItUnlimited()
    {
        var unit = new Unit { ObjId = 1, Level = 50 };
        TestUnitModifier.Apply(unit, UnitAttribute.MaxCombatResource, -5);
        await Assert.That(unit.AddCombatResource(FightResourceId, 10)).IsEqualTo(0);
    }
