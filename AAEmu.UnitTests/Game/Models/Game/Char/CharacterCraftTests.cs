...
public class CharacterCraftTests
{
    ...
    public async Task Craft_RejectsInvalidBatchCount(int count)
    {
        ...
        var started = context.CharacterCraft.TryStart(context.Craft, count, context.Doodad.ObjId);
    ...
    public async Task Craft_RejectsCraftOutsideActivePack()
    {
        ...
        var started = context.CharacterCraft.TryStart(context.Craft, 1, context.Doodad.ObjId);
    ...
    public async Task Craft_MultiFunctionStationUsesRecipeFunctionPermission(byte permission, bool expected)
    {
        ...
        await Assert.That(context.CharacterCraft.TryStart(context.Craft, 1, context.Doodad.ObjId)).IsEqualTo(expected);
    ...
    public async Task Craft_StartStopStart_AllowsNewSessionAndIgnoresOldCancellation()
    {
        ...
        await Assert.That(crafting.TryStart(context.Craft, 1, context.Doodad.ObjId)).IsTrue();
        ...
        await Assert.That(crafting.TryComplete(firstSkill, out _)).IsFalse();
    ...
    public async Task EndCraft_RejectsChangedFunctionGroup()
    {
        ...
        context.CharacterCraft.TryComplete(GetActiveSkill(context.CharacterCraft), out _);
    ...
    public async Task EndCraft_RejectsSkillOtherThanOriginatingInstance()
    {
        ...
        var completed = context.CharacterCraft.TryComplete(forgedSkill, out _);
        var repeated = context.CharacterCraft.TryComplete(forgedSkill, out _);
        ...
        byte permission = 0)
    {
        ...
        var craft = new Craft
        {
            ...
            SkillId = skillId,
            Enable = true,
            AllowEmptyMaterials = true,
            CraftProducts = Enumerable.Range(0, autoEquipProductRows)
                .Select(_ => new CraftProduct { ItemId = productItemId, Amount = productAmount, Rate = 100 })
                .ToList()
        };
        craft.CraftPackIds.Add(craftInPack ? craftPackId : craftPackId + 1);
        ...
        ItemTemplate productTemplate = freshnessGroupId > 0
            ...
            : new ItemTemplate { Id = productItemId, MaxCount = 1000 };
        ...
        skillManager.GetSkillTemplate(skillId).Returns(new SkillTemplate { Id = skillId, MaxRange = maxRange, Effects = [new SkillEffect { Template = new CraftEffect() }] });
        ...
        var bag = new ItemContainer(0, SlotType.Inventory, false, character) { ContainerSize = 100 };
        typeof(Inventory).GetProperty(nameof(Inventory.Bag))!.SetValue(character.Inventory, bag);
    ...
    {
        SetPrivateField(context.CharacterCraft, "_currentCraft", context.Craft);
        SetPrivateField(context.CharacterCraft, "_remainingCount", 1);
        SetPrivateField(context.CharacterCraft, "_doodadId", context.Doodad.ObjId);
        SetPrivateField(context.CharacterCraft, "_stationPhase", originalFunctionGroupId);
        SetPrivateField(context.CharacterCraft, "_productionZoneGroupId", context.ProductionZoneGroupId);
        SetPrivateField(context.CharacterCraft, "_activeSkill", skill ?? new Skill(new SkillTemplate
            { Id = context.Craft.SkillId, Effects = [new SkillEffect { Template = new CraftEffect() }] }));
    }

    private static Skill GetActiveSkill(CharacterCraft state) =>
        (Skill)typeof(CharacterCraft).GetField("_activeSkill", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(state);
        ...
        : CharacterCraft(owner, craftManager, doodadManager, skillManager, itemManager, zoneManager)
    {
        ...
        protected override SkillResult UseSkill(Skill skill, SkillCaster caster, SkillCastTarget target, SkillObject options, out ushort shortResult, out uint uintResult)
        {
            shortResult = 0;
            uintResult = 0;
            LastSkill = skill;
