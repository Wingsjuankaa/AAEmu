...
{
    [Test]
    public async Task ForSkillStarted_KeepsTheNativeRequestContext()
    {
        ...
        await Assert.That(extra.Flag).IsEqualTo(SkillObjectType.HousingRebuilding);
    }
    ...
    public async Task ForSkillStarted_EmptyHousingId_IsABareSkillObject()
    {
        ...
        await Assert.That(extra).IsNotTypeOf<SkillObjectHousingRebuilding>();
