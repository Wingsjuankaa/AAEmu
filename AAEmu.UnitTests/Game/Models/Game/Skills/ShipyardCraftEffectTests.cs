...
public class ShipyardCraftEffectTests
{
    ...
    public async Task Construction_ChargesSuccessfulActionAndRejectsWrongSkill(bool wrongSkill)
    {
        ...
        var ended = 0;
        skill.Callback = () => ended++;
        skill.EndSkill(character);
        skill.EndSkill(character); // Duplicate completion must neither debit nor publish twice.
    ...
    private sealed class LaborCharacter : CharacterMock
    {
        ...
        internal override bool TrySpendLabor(int cost, int actabilityId)
        {
            if (!HasLaborPower(cost)) return false;
            ChangeLabor(checked((short)-cost), actabilityId);
            return true;
        }
