...
public class SkillAreaRulesTests
{
    ...
    [Test]
    public async Task AreaAngle_IsAFullSweep()
    {
        await Assert.That(SkillAreaRules.ConeHalfAngle(90, 0)).IsEqualTo(45d);
        await Assert.That(SkillAreaRules.ConeHalfAngle(180, 40)).IsEqualTo(90d);
        await Assert.That(SkillAreaRules.ConeHalfAngle(5, 0)).IsEqualTo(2.5d);
    }

    [Test]
    public async Task FrontAngle_OnlyAppliesWhenTheAreaAngleIsTheDefault()
    {
        await Assert.That(SkillAreaRules.ConeHalfAngle(360, 40)).IsEqualTo(20d);
        await Assert.That(SkillAreaRules.ConeHalfAngle(365, 180)).IsEqualTo(90d);
    ...
    {
        // A 180° frontal sweep accepts +/-90, using the production conversion.
        var halfAngle = SkillAreaRules.ConeHalfAngle(180, 0);
