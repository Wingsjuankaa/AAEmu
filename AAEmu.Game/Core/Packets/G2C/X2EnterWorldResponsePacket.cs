    ...
    : GamePacket(SCOffsets.X2EnterWorldResponsePacket, 5)
{
    public override PacketStream Write(PacketStream stream)
    {
        ...
        // Preserve upstream's authority values. This is a native bitmask, not an access level:
        // bit 0x04 authorizes native GM commands (dev RVA 0x4C0520) and makes character
        // creation editor-only (dev RVA 0x4F18B0; release RVA 0x356D20). Server-side
        // gmFlag cannot replace that client permission. See Docs/AA10CharacterCreationGmAuthority_es.md.
        stream.Write(gm ? 101u : 1u);                // authority (i32)
        return stream;
    }
}
