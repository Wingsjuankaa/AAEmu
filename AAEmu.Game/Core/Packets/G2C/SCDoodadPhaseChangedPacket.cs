...
public class SCDoodadPhaseChangedPacket : GamePacket
{
    ...
    private readonly uint? _personalPhase;
    private readonly uint _funcGroupId;
    private readonly IItemManager _itemManager;

    public SCDoodadPhaseChangedPacket(Doodad doodad) : this(doodad, doodad.FuncGroupId, false)
    {
    }

    private SCDoodadPhaseChangedPacket(Doodad doodad, uint funcGroupId, bool personal) : base(SCOffsets.SCDoodadPhaseChangedPacket, 1)
    {
        _doodad = doodad;
        _funcGroupId = funcGroupId;
        _personalPhase = personal ? funcGroupId : null;
    ...
    public SCDoodadPhaseChangedPacket(Doodad doodad, uint personalPhase) : this(doodad, personalPhase, true)
    {
    ...
    public override PacketStream Write(PacketStream stream)
    {
        ...
        stream.Write(_personalPhase.HasValue ? 0 : _doodad.Data);
        stream.Write(_personalPhase.HasValue ? 0u : _doodad.TimeLeft); // growing
        stream.Write(_personalPhase.HasValue ? -1 : _doodad.PuzzleGroup);
        var itemManager = _itemManager ??
                          (_personalPhase.HasValue || _doodad.ItemId == 0 ? null : ItemManager.Instance);
        var goods = default(DoodadPhysicalGoods);
        var isGoods = !_personalPhase.HasValue && itemManager != null && DoodadPhysicalGoods.TryResolve(_doodad, itemManager, out goods);
        stream.Write(_personalPhase.HasValue ? 0u : isGoods ? goods.ItemTemplateId : _doodad.ItemTemplateId);
