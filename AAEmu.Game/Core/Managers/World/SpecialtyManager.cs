    ...
    IOptions<AppConfiguration> options) : Singleton<SpecialtyManager>, ISpecialtyManager
{
    ...
    private void PublishCommittedPurchase(Character player, SpecialtyPurchaseWrite write, int laborCost)
    {
        ...
        var publish = new List<Action>
        {
            ...
            () => player.ApplyCommittedLaborSpend(write.NewLabor - write.ExpectedLabor, write.NewLocalLabor - write.ExpectedLocalLabor,
        ...
        try
        {
            if (laborCost > 0)
            {
                player.ApplyCommittedLaborSpend(
                    write.NewLabor - write.ExpectedLabor, write.NewLocalLabor - write.ExpectedLocalLabor,
                    actabilityGroupId,
    ...
    internal static bool IsSelfTarget(uint packetId, uint activeId) => packetId == 0 || packetId == activeId;
    internal readonly record struct SpecialtyGoldPriceCalculation(double FreshnessAdjustedBasePrice, double PriceBeforeInterest, double Interest, int TotalPayout);
}
