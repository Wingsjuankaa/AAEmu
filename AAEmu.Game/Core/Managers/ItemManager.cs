...
public class ItemManager(ISkillManager skillManager, IItemIdManager itemIdManager, IContainerIdManager containerIdManager, ILocalizationManager localizationManager, ITaskManager taskManager, IWorldManager worldManager) : Singleton<ItemManager>, IItemManager
{
    ...
    {
        return ItemAwakeningCalculator.TryCalculateTotalExperience(
            category,
            grade,
            currentExp,
            _gradesOrdered.Values,
            out var total)
            ? total
            : currentExp;
    }
    ...
    /// </summary>
    /// <remarks>
    /// A grade advances once its <c>grade_exp</c> is paid. A grade that costs nothing is not on the
    /// ladder and ends the climb. Grades are walked by <c>grade_order</c> rather than by id, since Poor
    /// sorts below Basic.
    /// <para>
    /// <c>max_evolving_grade</c> is the terminal grade for the current category. Its final bar can still
    /// accept EXP, but once full it is saturated instead of promoting into the grade reserved for an
    /// awakened tier. Leftover EXP is otherwise carried: awakening replays the remainder onto the next
    /// ladder.
    /// </para>
    /// </remarks>
    public (byte Grade, int RemainingExp) SpendEvolvingExp(ItemRndAttrCategory category, byte startGrade, int exp)
    {
        return ItemSynthesisCalculator.TryResolveGrades(
            category,
            startGrade,
            0,
            exp,
            GetGradeTemplate,
            GetGradeTemplateByOrder,
            out var grade,
            out var remaining)
            ? (grade, remaining)
            : (startGrade, exp);
    ...
    {
        var resolution = ItemRandomAttributeResolver.ResolveForSynthesis(
            category, grade, [], maximum => Random.Shared.Next(maximum));
        return resolution.IsValid ? resolution.GroupIds.ToList() : [];
    }
    ...
    {
        return ItemAwakeningCalculator.SelectMapping(
            group, item?.TemplateId ?? 0, item?.Grade ?? 0, preferredMappingId);
    }
                ...
                using (var reader = new SQLiteWrapperReader(sqliteReader))
                {
                    while (reader.Read())
                    {
                        var category = new ItemRndAttrCategory
                        {
                            ...
                            MaxEvolvingGrade = reader.GetInt32("max_evolving_grade", -1),
