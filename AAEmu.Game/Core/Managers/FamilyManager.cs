    ...
    IFamilyPurchaseService familyPurchaseService, IGameDataManager gameDataManager) : Singleton<FamilyManager>, IFamilyManager
{
    ...
    public void AddExperience(Character source, uint amount) => TryAddExperienceCore(source, amount);

    private bool TryAddExperienceCore(Character source, uint amount)
    {
        Family changedFamily = null;
        using (var persistence = new PersistenceOperation())
        {
    ...
    public bool TryAddExperience(Character character, int point) =>
        CanAddExperience(character, point) &&
        (point == 0 || TryAddExperienceCore(character, checked((uint)point)));
