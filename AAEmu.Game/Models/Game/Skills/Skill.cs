        ...
        out uint skillResultValueUInt)
    {
        ...
        if (caster is Character or Units.Mate)
        {
            var crowdControl = CrowdControlRules.RejectCast(CrowdControlRules.ReadState(unit),
                SkillUseConditionRules.HasBit(Template.UseConditionBits, SkillUseConditionRules.SourceStun),
                SkillUseConditionRules.HasBit(Template.UseConditionBits, SkillUseConditionRules.SourceSleep),
                SkillUseConditionRules.HasBit(Template.UseConditionBits, SkillUseConditionRules.SourceSilence));
...
#pragma warning restore CA1508 // Avoid dead conditional code
    ...
    private void DelayActiveCast(Unit victim, int delayMilliseconds)
    {
        ...
        victim.SkillTask = new CastTask(this, victim, _activeCasterCaster, _activeTarget, _activeTargetCaster,
            _activeSkillObject);
        var remaining = running.TriggerTime - DateTime.UtcNow;
        if (remaining < TimeSpan.Zero)
            remaining = TimeSpan.Zero;
        TaskManager.Instance.Schedule(victim.SkillTask, remaining + TimeSpan.FromMilliseconds(delayMilliseconds));
    }
