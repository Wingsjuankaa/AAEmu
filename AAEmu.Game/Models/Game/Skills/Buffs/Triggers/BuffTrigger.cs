...
public class BuffTrigger
{
    ...
    private void Fire(EventArgs eventArgs, int delayMs)
    {
        ...
        if (eventArgs is OnKillArgs { Killer: null } or OnKillArgs { Victim: null })
            return;
        if (template.Kind is BuffEventTriggerKind.Death or BuffEventTriggerKind.KillAny &&
            !UnitRequirementsGameData.Instance.CanApplyBuffTrigger(template, owner,
                eventArgs is OnKillArgs kill ? kill.Victim : owner))
            return;
        ...
        void ApplyEffect()
        {
            ...
            if (template.EffectId != 0 && source.GetOwnerCharacter() is { } actor)
                ...
                {
                    Type = QuestObjectiveEventType.EffectFire, Actor = actor,
                    EffectId = template.EffectId, Amount = 1
                }, true);
        }
