...
/// <summary>Bridge for upstream callers to the native r575 type-7 request context.</summary>
public static class HousingRebuildSkillCast
{
    public static uint RequestedHousingId(SkillObject skillObject) =>
        skillObject is SkillObjectHousingRebuilding rebuild ? rebuild.TargetHousingId : 0;

    public static SkillObject ForSkillStarted(uint housingId) => housingId == 0
        ? new SkillObject()
        : new SkillObjectHousingRebuilding { Flag = SkillObjectType.HousingRebuilding, TargetHousingId = housingId };
}
