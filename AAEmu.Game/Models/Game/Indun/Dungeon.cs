...
public class Dungeon : IPreparedIndunInstance
{
    ...
    public bool InvitePlayer(Character character)
    {
        ...
        character.SendPacket(
            new SCAppliedToInstantGamePacket(_indunZone.InstanceId));
        character.SendPacket(
            new SCInviteToInstantGamePacket(
                unchecked((uint)invitationTime),
                _zoneInstanceId,
                0,
                1,
                1,
    ...
    private void OnDungeonLeave(object sender, OnDungeonLeaveArgs args)
    {
        ...
        // Bound copies stay up so the outside portal can re-enter or reset (ì´ˆê¸°í™”).
    ...
    /// <summary>Disconnect must not ì´ˆê¸°í™”. Relog re-enters the same copy.</summary>
