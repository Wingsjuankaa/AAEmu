...
namespace AAEmu.Game.Models.Game.Units
{
    public class Unit : BaseUnit
    {
        ...
        public virtual void DoDie(Unit killer, KillReason killReason)
        {
            ...
            {
                // Preserve the exact lethal-combat closure accepted by the
                // AA8 r558734 client in the last known-good 20:19 image.
                //
                // SCUnitAiAggro is an immediate (level 1) notification.  A
                // later experiment changed its owner to the dead NPC and
                // removed both level-5 SCCombatCleared notifications.  Every
                // captured death with that experimental sequence ended with
                // the client closing its game socket.  Do not reinterpret the
                // generic field name `npcId` here: the live AA8 contract is
                // the observed packet transaction, including its ordering.
                // Before the 2026-08-07 aggro reconstruction this lethal
                // clear was serialized in DD05.  Moving it to the immediate
                // level-1 channel coincided with the AA8 client stopping its
                // C2S stream after NPC death while ordinary level-1 updates kept
                // working. Preserve the historical transaction boundary for
                // the empty combat-closure form only.
                killer.BroadcastPacket(SCUnitAiAggroPacket.CreateCombatClear(killer.ObjId), true);
