...
public class ItemEnchantGameData : Singleton<ItemEnchantGameData>, IGameDataLoader
{
    ...
    {
        return checked((uint)(GetRndAttrProperty(categoryId, grade)?.GradeExp ?? 0));
    }
    ...
    public List<uint> RollRndAttrGroups(uint categoryId, byte grade, IReadOnlyCollection<uint> held = null)
    {
        ...
        foreach (var set in sets)
        {
            ...
            foreach (var group in groups)
            {
                ...
                else if (taken.Add(checked((short)group.UnitAttributeId)))
                {
                    result.Add(group.Id);
                    picks--;
                }
            ...
            while (picks > 0 && candidates.Count > 0)
            {
                ...
                if (!taken.Add(checked((short)group.UnitAttributeId)))
        ...
        uint sameBundleAs = 0)
    {
        ...
        foreach (var set in sets)
        {
            ...
            foreach (var group in groups)
            {
                if (!taken.Contains(checked((short)group.UnitAttributeId)))
                    candidates.Add(group);
            }
        }
    ...
    public List<short> GetRndAttrAttributes(IEnumerable<uint> groupIds)
    {
        ...
        foreach (var groupId in groupIds)
        {
            ...
            if (group != null)
                attributes.Add(checked((short)group.UnitAttributeId));
        }
    ...
    public ItemRndAttrUnitModifier GetRndAttrModifier(uint groupId, byte grade, uint exp = 0)
    {
        ...
        {
            Attribute = checked((short)group.UnitAttributeId),
            ModifierType = checked((byte)group.UnitModifierTypeId),
            Value = (int)value
        };
    }
        ...
        using (var command = connection.CreateCommand())
        {
            ...
            while (reader.Read())
            {
                ...
                var group = new ItemRndAttrUnitModifierGroup
                {
                    ...
                    UnitAttributeId = (uint)attributeId,
